...
class TicketTest < ActiveSupport::TestCase
  ...
  test "test that the submitter is stored properly" do
    ...
    ticket = build(:ticket, :submitter => user)
  ...
  test "test that the worker is stored properly" do
    user = build(:user, :first_name => 'Brent', :last_name => 'Montague')
    ticket = build(:ticket, :worker => user)
  ...
  test "email on status changed to resolved through comment" do
    ticket = create(:ticket)
    comment = build(:comment, :ticket_status => "Resolved")
    reset_email
    comment.save
    assert_not_nil last_email.subject
    assert last_email.subject.include?(APP_CONFIG['resolved_notify_subject'])
  ...
  test "email on worker change" do
    user1 = create(:user)
    user2 = create(:user)
    ticket = create(:ticket, :worker => user1)
    reset_email
    ticket.worker = user2
    ticket.save
